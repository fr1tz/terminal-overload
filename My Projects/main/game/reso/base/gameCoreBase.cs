...
function GameCoreBase::prepareMissionLoad(%game)
{
   ...
   requiredContent_addFromPattern("content/reso/shapes/*");   
...
function GameCoreBase::initGameVars(%game)
{
   ...
   $Game::defaultPlayerClass = "Camera";
   $Game::defaultPlayerDataBlock = "FrmSatellite";
   $Game::defaultPlayerSpawnGroups = "SatelliteSpawnPoints";
...
function GameCoreBase::prepareClient(%game, %client)
{
   ...
   %files = "xa/notc/core/client/base/v1/exec" TAB
            ...
            "reso/gui/SatelliteGui/v1/exec" TAB
            ...
            "xa/notc/deathmatch/client/gui/PlayerList/v1/exec" TAB
            "reso/ci/v1/exec";
...
function GameCoreBase::onUnitDestroyed(%game, %obj)
{
   ...
   echo("GameCoreBase::onUnitDestroyed():" SPC %obj);
}
...
function GameCoreBase::onControllerConnected(%game, %controller)
{
   //echo(%game @"\c4 -> "@ %game.class @" -> GameCoreBase::onControllerConnected");	
   %controller.initGrid(4, 2, 3);
   %controller.initGrid(3, 4, 6);
   %controller.initGrid(2, 8, 12);
   %controller.initGrid(1, 8, 12);
   %controller.initGrid(0, 8, 12);
     
   %controller.actionWidget = createWidget(%controller, "ActionWidget");
   %controller.addWidget(%controller.actionWidget, 0, "0 0", "N");  
       
   RESO::spawnDisc(%controller);
}

function GameCoreBase::onControllerDisconnected(%game, %controller)
{
   //echo(%game @"\c4 -> "@ %game.class @" -> GameCoreBase::onControllerDisconnected");	
   %controller.actionWidget.delete();
}
