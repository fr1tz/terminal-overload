...
function createServer(%gameType, %args)
{
   ...
   $Server::GameType = "RESO" SPC "\c1" @ $GameVersionString;
   ...
   $Server::RESO::Mutator::AS = false;
   $Server::RESO::Mutator::AM = false;
   $Server::RESO::Mutator::VAMP = false;
   ...
   for(%i = 0; %i < getWordCount(%args); %i++)
   {
      ...
      switch$(%arg)
      {
         ...
         case "-as":
            $Server::RESO::Mutator::AS = true;
            
         case "-am":
            $Server::RESO::Mutator::AM = true;
            
         case "-vamp":
            $Server::RESO::Mutator::VAMP = true;
   ...
   $Server::RESO::MutatorList = "";
   if($Server::RESO::Mutator::AS)
      $Server::RESO::MutatorList = $Server::RESO::MutatorList SPC "AS";
   if($Server::RESO::Mutator::AM)
      $Server::RESO::MutatorList = $Server::RESO::MutatorList SPC "AM";
   if($Server::RESO::Mutator::VAMP)
      $Server::RESO::MutatorList = $Server::RESO::MutatorList SPC "VAMP";
   $Server::RESO::MutatorList = trim($Server::RESO::MutatorList);
   
   if($Server::RESO::MutatorList $= "")
      $Server::GameType = $Server::GameType SPC "\c2Casual";
   else if($Server::RESO::MutatorList $= "AS AM VAMP")
      $Server::GameType = $Server::GameType SPC "\c5Advanced";
   else
      $Server::GameType = $Server::GameType SPC "\c6Variant";

   %modeVariant = "";
   if($Server::RESO::MutatorList $= "AS AM VAMP")
   {
      %modeVariant = "(Advanced)";
   }
   else if(getWordCount($Server::RESO::MutatorList) == 1)
   {
      if($Server::RESO::MutatorList $= "AS")
         %modeVariant = "(Advanced Shooting)";
      else if($Server::RESO::MutatorList $= "AM")
         %modeVariant = "(Advanced Movement)";
      else if($Server::RESO::MutatorList $= "VAMP")
         %modeVariant = "(V-AMP)";
   }
   else if(getWordCount($Server::RESO::MutatorList) >= 2)
   {
   ...
   // The controllers module provides server-side support for controllers
   ...
   $Server::RESO::Mode = %mode;
   $Server::RESO::Prefs = %prefs;
   ...
   %level = "content/reso/mis/" @ %map @ "/mission.mis";
...
function resetServerDefaults()
{
   ...
   exec( "./defaults.cs" );
   if(isFile($Server::RESO::Prefs))
      exec($Server::RESO::Prefs);
