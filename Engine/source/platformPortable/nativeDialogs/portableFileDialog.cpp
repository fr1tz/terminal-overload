...
{  
   ...
   AssertFatal(canvas && dialog, "");

   bool haveWindowMgr = false;

#ifdef TORQUE_SDL
   PlatformWindowManagerSDL* windowMgrSDL = dynamic_cast<PlatformWindowManagerSDL*>(WindowManager);
   if(windowMgrSDL)
   {
      haveWindowMgr = true;
...
#if TORQUE_OS_WIN
   Win32WindowManager* windowMgrWin32 = dynamic_cast<Win32WindowManager*>(WindowManager);
   if(windowMgrWin32)
   {
      haveWindowMgr = true;
      ...
      {
         Journal::_Dispatching = false;
         windowMgrWin32->_process();
         canvas->renderFrame(false, true);
      }
      Journal::_Dispatching = isDispatch;
   }
#else
   X11WindowManager* windowMgrX11 = dynamic_cast<X11WindowManager*>(WindowManager);
   if(windowMgrX11)
   {
      haveWindowMgr = true;
   ...
   AssertFatal(haveWindowMgr, "No window manager!");
