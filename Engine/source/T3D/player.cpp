...
bool PlayerData::preload(bool server, String &errorStr)
{
   ...
   if( !server )
   {
      ...
      for(int i=0; i < NumSlideEmitters; i++)
         if(!slideEmitter[i] && slideEmitterId[i] != 0)
            Sim::findObject(SimObjectId(slideEmitterId[i]), slideEmitter[i]);

      for(int i=0; i < NumSkidEmitters; i++)
         if(!skidEmitter[i] && skidEmitterId[i] != 0)
            Sim::findObject(SimObjectId(skidEmitterId[i]), skidEmitter[i]);
   }
...
void PlayerData::packData(BitStream* stream)
{
   ...
   for(U32 i = 0; i < NumSlideEmitters; i++)
      if (stream->writeFlag(slideEmitter[i]))
         stream->writeRangedU32(slideEmitter[i]->getId(),DataBlockObjectIdFirst,DataBlockObjectIdLast);

   for(U32 i = 0; i < NumSkidEmitters; i++)
      if (stream->writeFlag(skidEmitter[i]))
         stream->writeRangedU32(skidEmitter[i]->getId(),DataBlockObjectIdFirst,DataBlockObjectIdLast);
...
void PlayerData::unpackData(BitStream* stream)
{
   ...
   stream->read(&footPuffRadius);
	...
	{
		if(stream->readFlag())
			slideEmitterId[i] = (S32)stream->readRangedU32(DataBlockObjectIdFirst,DataBlockObjectIdLast);
   }

   for(U32 i = 0; i < NumSkidEmitters; i++)
   {
      if(stream->readFlag())
         skidEmitterId[i] = (S32)stream->readRangedU32(DataBlockObjectIdFirst,DataBlockObjectIdLast);
   }
