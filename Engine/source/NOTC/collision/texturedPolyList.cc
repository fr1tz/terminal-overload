...
bool TexturedPolyList::allowClipping = true;

//----------------------------------------------------------------------------

TexturedPolyList::TexturedPolyList()
...
bool TexturedPolyList::isEmpty() const
{
...
U32 TexturedPolyList::addPoint(const Point3F& p)
{
    return addPointAndNormal( p, Point3F::Zero );
}

U32 TexturedPolyList::addPointAndNormal(const Point3F& p, const Point3F& normal)
{
...
U32 TexturedPolyList::addPlane(const PlaneF& plane)
{
...
void TexturedPolyList::begin(BaseMatInstance* material,U32 surfaceKey)
{
   mPolyList.increment();
   ...
   poly.polyFlags = 0;
   if(TexturedPolyList::allowClipping)
...
void TexturedPolyList::vertex(U32 vi)
{
...
void TexturedPolyList::end()
{
   PROFILE_SCOPE( TexturedPolyList_Clip );
...
void TexturedPolyList::memcpy(U32* dst, U32* src,U32 size)
{
   U32* end = src + size;
   while (src != end)
      *dst++ = *src++;
...
void TexturedPolyList::cullUnusedVerts()
{
   PROFILE_SCOPE( TexturedPolyList_CullUnusedVerts );
...
void TexturedPolyList::triangulate()
{
   PROFILE_SCOPE( TexturedPolyList_Triangulate );
...
void TexturedPolyList::generateNormals()
{
   PROFILE_SCOPE( TexturedPolyList_GenerateNormals );
...
void TexturedPolyList::addCoat(F32 thickness)
{
	...
	for(U32 i = 0; i < mVertexList.size(); i++)
	{
		...
		vertex.point += vertex.normal * thickness;
	}
}

void TexturedPolyList::addTexture(S32 squareSize)
{
	Poly* p;
	for (p = mPolyList.begin(); p < mPolyList.end(); p++)
	{
		...
		{
			U32 vertexIndex = mIndexList[p->vertexStart+i];
			Vertex& vertex = mVertexList[vertexIndex];
			vertex.texCoord.x = vertex.point.x/squareSize;
			vertex.texCoord.y = vertex.point.y/squareSize;
		}
	}
}
