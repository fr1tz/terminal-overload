...
ConsoleFunctionGroupBegin( DateAndTime, "Functions to access date & time" );

ConsoleFunction( getTime, S32, 1, 1, "Returns current time" )
{
	return Platform::getTime();
}

ConsoleFunction( getDateAndTime, const char*, 1, 1, "Returns current date & time" )
{
	...
	dSprintf(ret, 512, "%s.%d.%d-%02d:%02d:%02d",
               month,
               lt.monthday,
               lt.year + 1900,
               lt.hour,
...
ConsoleFunctionGroupEnd( DateAndTime );
