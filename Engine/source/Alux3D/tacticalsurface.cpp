...
ConsoleMethod( TacticalSurface, getNumObjects, S32, 2, 2, "")
{
   return object->getNumTacticalSurfaceObjects();
}

ConsoleMethod( TacticalSurface, getObject, S32, 3, 3, "(int idx)")
{
   S32 index = dAtoi(argv[2]);

   if (index >= object->getNumTacticalSurfaceObjects() || index < 0)
      return -1;
   else
...
IMPLEMENT_CO_DATABLOCK_V1(TacticalSurfaceData);

TacticalSurfaceData::TacticalSurfaceData()
{
	tickPeriodMS = 100;
...
bool TacticalSurfaceData::onAdd()
{
   if (!Parent::onAdd())
      return false;
...
void TacticalSurfaceData::initPersistFields()
{
	Parent::initPersistFields();

	addField("tickPeriodMS",        TypeS32,      Offset(tickPeriodMS, TacticalSurfaceData));
	addField("colorChangeTimeMS",   TypeS32,      Offset(colorChangeTimeMS, TacticalSurfaceData));
	addField("colors",              TypeColorF,   Offset(colors, TacticalSurfaceData), MaxColors );
	addField("texture",             TypeFilename, Offset(texture,TacticalSurfaceData));
	addField("borderTexture",       TypeFilename, Offset(borderTexture,TacticalSurfaceData));
}


//--------------------------------------------------------------------------

void TacticalSurfaceData::packData(BitStream* stream)
{
...
void TacticalSurfaceData::unpackData(BitStream* stream)
{
...
bool TacticalSurfaceData::preload(bool server, char errorBuffer[256])
{
...
S32 TacticalSurface::sRenderMode = Full;
bool TacticalSurface::sRenderTerrainDebug = false;

IMPLEMENT_CO_NETOBJECT_V1(TacticalSurface);
TacticalSurface::TacticalSurface()
{
   mNetFlags.set(ScopeAlways);

   mTypeMask = TacticalSurfaceObjectType; // discard parent masks
...
void TacticalSurface::initPersistFields()
{
	Parent::initPersistFields();

	addField("showOnMinimap",    TypeBool, Offset(mShowOnMinimap,TacticalSurface));

	addField("renderInteriors",   TypeBool, Offset(mRenderInteriors,TacticalSurface));
	addField("renderTerrain",     TypeBool, Offset(mRenderTerrain,TacticalSurface));
	addField("renderTerrainGrid", TypeBool, Offset(mRenderTerrainGrid,TacticalSurface));

	addField("borderBottom", TypeF32, Offset(mBorderWidth[0],TacticalSurface));
	addField("borderLeft",   TypeF32, Offset(mBorderWidth[1],TacticalSurface));
	addField("borderBack",   TypeF32, Offset(mBorderWidth[2],TacticalSurface));
	addField("borderFront",  TypeF32, Offset(mBorderWidth[3],TacticalSurface));
	addField("borderRight",  TypeF32, Offset(mBorderWidth[4],TacticalSurface));
	addField("borderTop",    TypeF32, Offset(mBorderWidth[5],TacticalSurface));
}

void TacticalSurface::consoleInit()
{
	Parent::consoleInit();

	Con::setIntVariable("$TacticalSurfaceRenderMode::None", None);
	Con::setIntVariable("$TacticalSurfaceRenderMode::BordersOnly", BordersOnly);
	Con::setIntVariable("$TacticalSurfaceRenderMode::Full", Full);

	Con::addVariable("Pref::TacticalSurface::RenderMode", TypeS32, &sRenderMode);
	Con::addVariable("TacticalSurface::RenderTerrainDebug", TypeS32, &sRenderTerrainDebug);
}

//--------------------------------------------------------------------------

void TacticalSurface::computePolys()
{
	...
	Container::FindCallback callback = &TacticalSurface::objectFound;
...
void TacticalSurface::computeGrid()
{
...
void TacticalSurface::clearGrid()
{
...
bool TacticalSurface::onNewDataBlock(GameBaseData* dptr)
{
   mDataBlock = dynamic_cast<TacticalSurfaceData*>(dptr);
...
void TacticalSurface::onDeleteNotify(SimObject* obj)
{
...
void TacticalSurface::inspectPostApply()
{
	setMaskBits(EditorMask);
}

void TacticalSurface::onEditorEnable()
{
	//mDebugMode = true;
}

void TacticalSurface::onEditorDisable()
{
	//mDebugMode = false;
}
...
void TacticalSurface::buildConvex(const Box3F& box, Convex* convex)
{
...
void TacticalSurface::setTransform(const MatrixF & mat)
{
...
ColorF TacticalSurface::getRenderColor()
{
	ColorF c = mCurrColor;
...
ColorF TacticalSurface::getZoneColor()
{
	ColorF color;
...
void TacticalSurface::setColor(U8 color1, U8 color2, F32 f)
{
	if(mColor1 != color1)
	{
		mColor1 = color1;
...
void TacticalSurface::flash(U8 color1, U8 color2, F32 f)
{
...
bool TacticalSurface::testObject(GameBase* enter)
{
   mClippedList.clear();
...
void TacticalSurface::potentialEnterObject(GameBase* enter)
{
...
void TacticalSurface::processTick(const Move* move)
{
...
void TacticalSurface::advanceTime(F32 dt)
{
...
bool TacticalSurface::prepRenderImage(SceneState* state, const U32 stateKey,
                              const U32 startZone, const bool modifyBaseState)
{
	...
	if(state->isObjectRendered(this))
	{
		...
		for(U32 faceidx = startFace; faceidx <= 5; faceidx++)
		{
			...
			//bool col = getContainer()->castRay(p,camPos,TacticalSurfaceObjectType,&rInfo); 
			//mIgnoreRayCast = true;
			//if(!col)
			{
				//
				...
				TacticalSurfaceFaceRenderImage* image = new TacticalSurfaceFaceRenderImage;
				image->obj = this;
...
void
TacticalSurface::updateFogCoords(TexturedPolyList& polyList, const Point3F& camPos)
{
...
void TacticalSurface::renderObject(SceneState* state, SceneRenderImage* image)
{ 
	AssertFatal(dglIsInCanonicalState(), "Error, GL not in canonical state on entry");

	TacticalSurfaceFaceRenderImage* faceImage = dynamic_cast<TacticalSurfaceFaceRenderImage*>(image);
	if(!faceImage)
		return;
...
void TacticalSurface::renderDebug(TacticalSurfaceFaceRenderImage* image)
{
	U32 face = image->face;
...
void TacticalSurface::renderInteriorPolyList(TexturedPolyList& polyList)
{
	return;
...
void TacticalSurface::renderBorderPolyList(TexturedPolyList& polyList)
{
	ColorF color = this->getRenderColor();
	if(&polyList == &mBorderPolys[5])
		color.alpha *= 0.5;
	glColor4fv(color);
...
void TacticalSurface::renderTerrainPolyList(TexturedPolyList& polyList)
{
...
void TacticalSurface::renderTerrainGridLines(VertexList& vertexList)
{
	glColor4fv          ( this->getRenderColor() );
...
bool TacticalSurface::castRay(const Point3F& start, const Point3F& end, RayInfo* info)
{
...
U32 TacticalSurface::packUpdate(NetConnection* con, U32 mask, BitStream* stream)
{
	...
	if( stream->writeFlag(mask & Color1Mask) )
		stream->writeInt(mColor1, TacticalSurfaceData::MaxColorsBits);

	if( stream->writeFlag(mask & Color2Mask) )
		stream->writeInt(mColor2, TacticalSurfaceData::MaxColorsBits);

	if( stream->writeFlag(mask & FactorMask) )
		writeRangedF32(stream, mFactor, 0, 1, 4);

	if( stream->writeFlag(mask & FlashColor1Mask) )
		stream->writeInt(mFlashColor1, TacticalSurfaceData::MaxColorsBits);

	if( stream->writeFlag(mask & FlashColor2Mask) )
		stream->writeInt(mFlashColor2, TacticalSurfaceData::MaxColorsBits);
...
void TacticalSurface::unpackUpdate(NetConnection* con, BitStream* stream)
{
   Parent::unpackUpdate(con, stream);
	...
	if( stream->readFlag() )
		mColor1 = stream->readInt(TacticalSurfaceData::MaxColorsBits);

	if( stream->readFlag() )
		mColor2 = stream->readInt(TacticalSurfaceData::MaxColorsBits);

	if( stream->readFlag() )
		mFactor = readRangedF32(stream, 0, 1, 4);

	if( stream->readFlag() )
		mFlashColor1 = stream->readInt(TacticalSurfaceData::MaxColorsBits);

	if( stream->readFlag() )
		mFlashColor2 = stream->readInt(TacticalSurfaceData::MaxColorsBits);
...
void TacticalSurface::objectFound(SceneObject* obj, void* key)
{
	TacticalSurface* zone = (TacticalSurface*)key;
...
ConsoleMethod( TacticalSurface, setColor, void, 5, 5, "(int color1, int color2, float f)")
{
	object->setColor(dAtoi(argv[2]), dAtoi(argv[3]), dAtof(argv[4]));
}

ConsoleMethod( TacticalSurface, flash, void, 5, 5, "(int color1, int color2, float f)")
{
	object->flash(dAtoi(argv[2]), dAtoi(argv[3]), dAtof(argv[4]));
}

ConsoleMethod( TacticalSurface, computeGrid, void, 2, 2, "")
{
